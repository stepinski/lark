...
// DataPoint is a single timestamped observation.
type DataPoint struct {
	Time time.Time
	// Value is the parsed float64. NaN if the raw value was non-numeric.
	Value float64
...
type Site struct {
	...
	SiteType string `json:"InternalName"`
}
...
// DateRange fetches data between start and end (format: "yyyy-MM-dd" or "yyyy-MM-ddTHH:mm:ss").
...
func (c *Client) authenticate(ctx context.Context) error {
	body, err := json.Marshal(tokenRequest{UserName: c.username, Password: c.password})
	if err != nil {
...
func (c *Client) fetchChunk(ctx context.Context, siteID, channelID int, p *queryParams) ([]DataPoint, error) {
	...
	for _, r := range resp.Resources {
		t, err := time.Parse("2006-01-02T15:04:05", r.DataTime)
		if err != nil {
			t, _ = time.Parse(time.RFC3339, r.DataTime)
		}
		v, err := strconv.ParseFloat(r.DataValue, 64)
		dp := DataPoint{Time: t.UTC(), Raw: r.DataValue}
		if err == nil {
			dp.Value = v
		} else {
			dp.Value = math.NaN()
...
// MultiChannelData fetches multiple channels concurrently.
...
func (e *MultiError) Error() string {
	...
	return fmt.Sprintf("%d channel errors: %v", len(e.Errors), e.Errors[0])
}
