...
func (m *Model) Fit(y []float64, exog [][]float64) error {
	...
	col := 0
	m.params.Intercept = coeffs[col]
	col++
	m.params.AR = make([]float64, o.P)
	for i := range m.params.AR {
		m.params.AR[i] = coeffs[col]
		col++
	}
	m.params.SAR = make([]float64, o.SP)
	for i := range m.params.SAR {
		m.params.SAR[i] = coeffs[col]
		col++
	}
	m.params.MA = make([]float64, o.Q)
	for i := range m.params.MA {
		m.params.MA[i] = coeffs[col]
		col++
	}
	m.params.SMA = make([]float64, o.SQ)
	for i := range m.params.SMA {
		m.params.SMA[i] = coeffs[col]
		col++
	}
	m.params.Exog = make([]float64, nExog)
	for i := range m.params.Exog {
		m.params.Exog[i] = coeffs[col]
		col++
	}
